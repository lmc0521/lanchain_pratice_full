...
from langchain_core.runnables import RunnableWithMessageHistory, configurable, ConfigurableFieldSpec
...
while input_text.lower()!='bye':
    if input_text:
        response=with_message_history.with_config(configurable={'user_id':user_id,'conversation_id':conversation_id}).invoke({'input':input_text})
        print(response)
    input_text=input('>>> ')